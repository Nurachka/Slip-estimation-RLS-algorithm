...
file_path = os.path.join(data_dir, 'gerono-cmd_vel_exp5.csv')
#file path for the imu data
file_path_imu = os.path.join(data_dir, 'gerono-imu1_exp5.csv')
...
#deleting the rows that are before 39.52 sec, because the cmd vel data starts at 12.45 sec
df_imu_filtered = df_imu_filtered[df_imu_filtered['time'] > '2025/07/11/14:10:39.520982']
#deleting the rows after 2025/07/11/14:10:59.171122 because cmd vel stops sending velocity after that
df_imu_filtered = df_imu_filtered[df_imu_filtered['time'] <='2025/07/11/14:10:59.171122']
...
# #deleting the row  of theta_unwrapped at the index 35, cause there is a discontinuity in the data
df_cmd_synced = df_cmd_synced.drop(index=35)
