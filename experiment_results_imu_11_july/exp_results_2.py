...
file_path = os.path.join(data_dir, 'gerono-cmd_vel_exp2.csv')
#file path for the imu data
file_path_imu = os.path.join(data_dir, 'gerono-imu1_exp2.csv')
...
#deleting the rows that are before 38.26 sec, because the cmd vel data starts at 12.45 sec
df_imu_filtered = df_imu_filtered[df_imu_filtered['time'] >= '2025/07/11/14:07:38.258400']
#deleting the rows after 2025/07/11/14:06:32.141455 because cmd vel stops sending velocity after that
df_imu_filtered = df_imu_filtered[df_imu_filtered['time'] <='2025/07/11/14:07:57.861537']
...
#deleting the row  of theta_unwrapped at the index 196, cause there is a discontinuity in the data
df_cmd_synced = df_cmd_synced.drop(index=196)
