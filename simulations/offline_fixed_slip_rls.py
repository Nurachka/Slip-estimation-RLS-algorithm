...
plt.plot(time_list, compensated_robot.theta_list, label='Compensated trajectory', linestyle='-.')
plt.xlabel('Time (s)')
...
plt.subplot(1, 2, 1)
plt.plot(time_list, error_comp, label='Tracking error (compensated vs target)')
plt.xlabel('Time (s)')
plt.ylabel('Error (m)')
...
plt.plot(time_list, estimationErrorCovarianceValues, label='Estimation Error Covariance')
plt.xlabel('Time (s)')
...
plt.plot(time_list, kalmanGains, label='Kalman Gain')
plt.xlabel('Time (s)')
...
plt.figure(figsize=(10, 4))
plt.subplot(1, 2, 1)
plt.plot(time_list, vel_right_ref, label='Reference right wheel velocity', linestyle='--')
plt.plot(time_list, vel_right_comp, label='Compensated right wheel velocity', linestyle='-.')
plt.xlabel('Time (s)')
...
plt.grid()
plt.subplot(1, 2, 2)
plt.plot(time_list, vel_left_ref, label='Reference left wheel velocity', linestyle='--')
plt.plot(time_list, vel_left_comp, label='Compensated left wheel velocity', linestyle='-.')
plt.xlabel('Time (s)')
plt.ylabel('Velocity (m/s)')
...
plt.plot(x_target_list, y_target_list)
plt.xlabel('X Position (m)')
plt.ylabel('Y Position (m)')
plt.legend()
plt.axis('equal')
...
#plot reference theta
ref_theta = robot.theta_list
plt.plot(time_list, ref_theta, label='$\\theta_d$')
plt.xlabel('Time (s)')
...
#plot reference angular velocity (derivative of reference theta)
ref_angular_velocity = [(ref_theta[i] - ref_theta[i-1]) / 0.05 for i in range(1, len(ref_theta))]
plt.plot(time_list[1:], ref_angular_velocity, label='$\\dot{\\theta_d}$')
plt.xlabel('Time (s)')
plt.ylabel('$\\dot{\\theta_d}$ (rad/s)')
plt.legend()
plt.grid()
plt.show()

#plot reference right wheel velocity
plt.plot(time_list, vel_right_ref, label='$v_{d,right}$')
plt.xlabel('Time (s)')
plt.ylabel('Velocity (m/s)')
plt.legend()
plt.grid()
...
#plot reference left wheel velocity
plt.plot(time_list, vel_left_ref, label='$v_{d,left}$')
plt.xlabel('Time (s)')
plt.ylabel('Velocity (m/s)')
plt.legend()
plt.grid()
