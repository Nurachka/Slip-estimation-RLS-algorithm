...
for k in range(n_steps):
    ...
    rls.predict_sim_with_forgetting_factor(
        theta_n, theta_prev, vel_right, vel_left, delta_t=DELTA_T, lam=LAMBDA
    )
    theta_prev = theta_n
...
true_slip_list  = [TRUE_SLIP if k >= SLIP_ONSET else 0.0 for k in range(n_steps)]
...
plt.axvline(SLIP_ONSET,   color='grey',   linestyle=':', linewidth=0.8, label=f'Slip onset (k={SLIP_ONSET})')
...
print(f"Position error after slip onset (MPC + RLS): min {error_comp[SLIP_ONSET:].min():.3f} m, max {error_comp[SLIP_ONSET:].max():.3f} m, mean {error_comp[SLIP_ONSET:].mean():.3f} m")
