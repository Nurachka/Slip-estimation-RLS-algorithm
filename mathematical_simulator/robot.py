...
class Robot:
    def __init__(self):
        ...
        self.slip = 0.26  # Slip factor, a value between 0 and 1
        self.noise_std_position = 0.025  # Standard deviation for position noise
        self.noise_std_orientation = 0.000436  # Standard deviation for orientation noise
        self.x_list = []
        self.y_list = []
        self.theta_list = []
        self.x_list_noised = []
    ...
    def forward_kinematics(self, vel_right, vel_left):
        x_prev = self.x_list[-1] if self.x_list else 0.0
        y_prev = self.y_list[-1] if self.y_list else 0.0
        theta_prev = self.theta_list[-1] if self.theta_list else 0.0
        
        v = (vel_right + vel_left) / 2.0  # Linear velocity
        omega = (vel_right - vel_left) / self.wheel_base  # Angular velocity
        # Update position and orientation
        x = v *(1-self.slip)* np.cos(theta_prev) * self.timestep + x_prev
        y = v *(1-self.slip) * np.sin(theta_prev) * self.timestep + y_prev
        theta = omega  * (1- self.slip)* self.timestep + theta_prev
